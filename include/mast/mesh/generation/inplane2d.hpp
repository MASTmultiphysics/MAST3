...
#ifndef __mast_mesh_generation_inplane_2d_h__
#define __mast_mesh_generation_inplane_2d_h__
...
/*!
 * This struct provides the methods to populate the mesh, load, Dirichlet conditions and design variables for
 * topology optimization over a quadrilateral domain, where a pressure load is applied on a fraction of the
 * top boundary length and displacements along both x- and y-axis are constrained on the left and right
 * edges.
 */
struct Inplane2D {
    ...
    template <typename ScalarType>
    using pressure_t        =  MAST::Mesh::Generation::Inplane2D::Pressure<ScalarType>;
    ...
    reference_volume(Context& c) {
        ...
        height  = c.input("height", "length of domain along y-axis", 1.0);
    ...
    init_analysis_mesh(Context& c,
        ...
        ny_divs = c.input("ny_divs", "number of elements along y-axis", 30);
    ...
    init_analysis_dirichlet_conditions(Context& c) {
        ...
        (libMesh::DirichletBoundary({1, 3}, {0, 1}, libMesh::ZeroFunction<real_t>()));
    }
    ...
    (Context                                               &c,
     MAST::Optimization::DesignParameterVector<ScalarType> &dvs) {
        ...
        height        = c.input("height", "length of domain along y-axis", 1.0),
        frac          = c.input("loadlength_fraction", "fraction of boundary length on which pressure will act", 0.2),
        filter_radius = c.input("filter_radius", "radius of geometric filter for level set field", 0.07),
...
#endif // __mast_mesh_generation_inplane_2d_h__
