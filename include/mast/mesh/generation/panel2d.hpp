...
#ifndef __mast_mesh_generation_panel_2d_h__
#define __mast_mesh_generation_panel_2d_h__
...
struct Panel2D {
    ...
    template <typename ScalarType>
    using pressure_t        =  MAST::Mesh::Generation::Panel2D::Pressure<ScalarType>;
                ...
                for (uint_t i=0; i<nx; i++) {
                    ...
                    *elem = libMesh::Elem::build(libMesh::QUAD4).release();
                    ...
                    if (i == 0 && j >= (1.-dirichlet_length_fraction) * ny)
                        boundary_info.add_side(elem, 3, 6);
