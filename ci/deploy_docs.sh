...
echo "Hello from the deploy_docs.sh in the deploy stage"
...
echo "DEPLOY: Transferring archive to https://mast3-temp-docs.mast-multiphysics.com" || exit
scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -q -r "${ARCHIVE_NAME}" ${WEBSITE_HOST_USER}@${WEBSITE_HOST_ADDRESS}:mast3-temp-docs.mast-multiphysics.com || exit
