...
if [ "${TRAVIS_OS_NAME}" = linux ]; then # Ubuntu Linux
  ...
  if [ "${TRAVIS_DIST}" = bionic ]; then # Ubuntu 18.04 Bionic Beaver
...
#        ctest --force-new-ctest-process --output-on-failure -L "SHORT" --timeout 60
    ...
    if [ ${CI_BUILD_DOCS} ]; then
      echo "HELLO FROM DOCUMENTATION BUILDING STEP...I'm not doing anything yet!" || exit
...
#      ctest --force-new-ctest-process --output-on-failure -L "SHORT" --timeout 60
  ...
  if [ "${LIBMESH_VERSION}" = "1.5.1" ]; then
    ...
    echo "INVALID LIBMESH_VERSION: ${LIBMESH_VERSION}"
    exit 1
  fi
