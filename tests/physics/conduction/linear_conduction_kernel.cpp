...
namespace LinearConductanceKernel {
...
struct Traits {
    ...
    using kernel_t          = typename MAST::Physics::Conduction::ConductionKernel<fe_var_t, prop_t, Dim, Context, true, true>;
};
...
struct ElemOps {
    ...
    k        (new typename Traits::conductance_t(3.5e2)),
    prop     (new typename Traits::prop_t),
    kernel_e (new typename Traits::kernel_t) {
        ...
        prop->set_conductance(*k);
        
        kernel_e->set_section_property(*prop);
        kernel_e->set_fe_var_data(*fe_var);
    }
    
    virtual ~ElemOps() {}

    inline uint_t n_dofs() const { return kernel_e->n_dofs();}
    ...
    inline void derivative(const ScalarFieldType& f,
        ...
        kernel_e->derivative(c, f, res, jac);
    }
    ...
    std::unique_ptr<typename Traits::conductance_t>  k;
    std::unique_ptr<typename Traits::prop_t>         prop;
    std::unique_ptr<typename Traits::kernel_t>       kernel_e;
    Context                                          c;
};
...
TEST_CASE("linear_conduction_kernel",
          "[2D][QUAD4][Conduction][Linear][ConductanceKernel]") {
    ...
    // residual sensitivity wrt k
    {
        res.setZero();
        jac.setZero();
        e_ops.derivative(*e_ops.k, res, &jac);
        
        ElemOps<traits_complex_t> e_ops_c;
        
        complex_step_derivative
        <typename traits_complex_t::conductance_t, traits_t, traits_complex_t>
        (e_ops_c, e.get(), *e_ops_c.k, sol, res_cs, jac_cs);
