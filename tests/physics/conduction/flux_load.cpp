...
struct Traits {
    ...
    using flux_load_t  = typename MAST::Physics::Conduction::SurfaceFluxLoad<fe_var_t, flux_t, area_t, Dim, Context>;
};
...
struct ElemOps {
    ...
    p        (new typename Traits::flux_t(2.e3)),
    A        (new typename Traits::area_t(0.25)),
    flux_e   (new typename Traits::flux_load_t) {
        ...
        flux_e->set_flux(*p);
        flux_e->set_section_area(*A);
        flux_e->set_fe_var_data(*fe_var);
    }
    
    virtual ~ElemOps() {}

    inline uint_t n_dofs() const { return flux_e->n_dofs();}
...
TEST_CASE("surface_flux_load",
          "[2D][QUAD4][Conduction][FluxLoad]") {
    ...
    for (uint_t s=0; s<e->n_sides(); s++) {
        ...
        // residual sensitivity wrt p
        {
            ...
            <typename traits_complex_t::flux_t, traits_t, traits_complex_t>
            (e_ops_c, e.get(), s, *e_ops_c.p, sol, res_cs, jac_cs);
