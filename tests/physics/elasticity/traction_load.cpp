...
struct Traits {
    ...
    using fe_s_data_t  = typename MAST::FEBasis::libMeshWrapper::FESideData<Dim, fe_basis_t, fe_shape_t>;
    using fe_var_t     = typename MAST::FEBasis::FEVarData<BasisScalarType, NodalScalarType, SolScalarType, Dim, Dim, Context, fe_shape_t>;
    using trac_scalar_t= typename MAST::Base::ScalarConstant<SolScalarType>;
    using traction_t   = MAST::Physics::Elasticity::Traction<trac_scalar_t, Dim, Context>;
    using area_t       = typename MAST::Base::ScalarConstant<SolScalarType>;
    using trac_load_t  = typename MAST::Physics::Elasticity::SurfaceTractionLoad<fe_var_t, traction_t, area_t, Dim, Context>;
};
...
struct ElemOps {
    ...
    t0       (new typename Traits::trac_scalar_t(2.e3)),
    t1       (new typename Traits::trac_scalar_t(2.5e3)),
    t2       (new typename Traits::trac_scalar_t(3.03)),
    t        (new typename Traits::traction_t(t0.get(),
                                              Traits::dim>1?t1.get():nullptr,
                                              Traits::dim>2?t2.get():nullptr)),
    A        (new typename Traits::area_t(0.25)),
    trac_e   (new typename Traits::trac_load_t) {
        ...
        trac_e->set_traction(*t);
        trac_e->set_section_area(*A);
        trac_e->set_fe_var_data(*fe_var);
    }
    
    virtual ~ElemOps() {}

    inline uint_t n_dofs() const { return trac_e->n_dofs();}
    
    inline void init_for_side(const libMesh::Elem* e,
                              const uint_t s) {
        
        c.elem = e;
        fe_data->reinit_for_side(c, s);
    }
    ...
    inline void derivative(const ScalarFieldType& f,
        ...
        trac_e->derivative(c, f, res, jac);
    }

    std::unique_ptr<typename Traits::fe_s_data_t>    fe_data;
    std::unique_ptr<typename Traits::fe_var_t>       fe_var;
    std::unique_ptr<typename Traits::trac_scalar_t>  t0, t1, t2;
    std::unique_ptr<typename Traits::traction_t>     t;
    std::unique_ptr<typename Traits::area_t>         A;
    std::unique_ptr<typename Traits::trac_load_t>    trac_e;
    Context                                          c;
};
...
TEST_CASE("linear_traction_load",
          "[2D][QUAD4][Elasticity][Linear][TractionLoad]") {
    ...
    for (uint_t s=0; s<e->n_sides(); s++) {
        
        ElemOps<traits_t> e_ops;
        e_ops.init_for_side(e.get(), s);
        ...
        // residual sensitivity wrt t0
        {
            res.setZero();
            jac.setZero();
            e_ops.derivative(*e_ops.t0, res, &jac);
            
            ElemOps<traits_complex_t> e_ops_c;
            
            complex_step_derivative
            <typename traits_complex_t::trac_scalar_t, traits_t, traits_complex_t>
            (e_ops_c, e.get(), s, *e_ops_c.t0, sol, res_cs, jac_cs);
            
            CHECK_THAT(MAST::Test::eigen_matrix_to_std_vector(res),
                       Catch::Approx(MAST::Test::eigen_matrix_to_std_vector(res_cs)));
        }

        // residual sensitivity wrt t1
        {
            res.setZero();
            jac.setZero();
            e_ops.derivative(*e_ops.t1, res, &jac);
            
            ElemOps<traits_complex_t> e_ops_c;
            
            complex_step_derivative
            <typename traits_complex_t::trac_scalar_t, traits_t, traits_complex_t>
            (e_ops_c, e.get(), s, *e_ops_c.t1, sol, res_cs, jac_cs);
        ...
        // residual sensitivity wrt A
        {
            ...
            (e_ops_c, e.get(), s, *e_ops_c.A, sol, res_cs, jac_cs);
