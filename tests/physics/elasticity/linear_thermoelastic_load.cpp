...
struct Traits {
    ...
    using temperature_t = typename MAST::Base::ScalarConstant<SolScalarType>;
    using temp_load_t   = typename MAST::Physics::Elasticity::LinearContinuum::ThermoelasticLoad<fe_var_t, temperature_t, alpha_t, prop_t, Dim, Context>;
};
...
struct ElemOps {
    ...
    alpha    (new typename Traits::alpha_t(2.e-4)),
    prop     (new typename Traits::prop_t),
    dt       (new typename Traits::temperature_t(2.e1)),
    therm_e  (new typename Traits::temp_load_t) {
        ...
        fe_data->fe_derivative().set_compute_dphi_dx(true);
        fe_data->fe_derivative().set_compute_Jac_inverse(true);
        ...
        therm_e->set_temperature(*dt);
        therm_e->set_coeff_thermal_expansion(*alpha);
        therm_e->set_section_property(*prop);
        therm_e->set_fe_var_data(*fe_var);
    }
    
    virtual ~ElemOps() {}

    inline uint_t n_dofs() const { return therm_e->n_dofs();}
    ...
    inline void derivative(const ScalarFieldType& f,
        ...
        therm_e->derivative(c, f, res, jac);
    }
    ...
    std::unique_ptr<typename Traits::alpha_t>        alpha;
    std::unique_ptr<typename Traits::prop_t>         prop;
    std::unique_ptr<typename Traits::temperature_t>  dt;
    std::unique_ptr<typename Traits::temp_load_t>    therm_e;
    Context                                          c;
};
...
TEST_CASE("linear_thermoelastic_load",
          "[2D][QUAD4][Elasticity][Linear][ThermoelasticLoad]") {
    ...
    // residual sensitivity wrt dt
    {
        res.setZero();
        jac.setZero();
        e_ops.derivative(*e_ops.dt, res, &jac);
        
        ElemOps<traits_complex_t> e_ops_c;
        
        complex_step_derivative
        <typename traits_complex_t::temperature_t, traits_t, traits_complex_t>
        (e_ops_c, e.get(), *e_ops_c.dt, sol, res_cs, jac_cs);
    ...
    // residual sensitivity wrt nu
    {
        ...
        complex_step_derivative
        <typename traits_complex_t::nu_t, traits_t, traits_complex_t>
    ...
    // residual sensitivity wrt alpha
    {
        res.setZero();
        jac.setZero();
        e_ops.derivative(*e_ops.alpha, res, &jac);
        
        ElemOps<traits_complex_t> e_ops_c;
        
        complex_step_derivative
        <typename traits_complex_t::alpha_t, traits_t, traits_complex_t>
        (e_ops_c, e.get(), *e_ops_c.alpha, sol, res_cs, jac_cs);
